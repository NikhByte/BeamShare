...
func TestLiveStreamPipeSSE(t *testing.T) {
	...
	req1, err := http.NewRequest(http.MethodGet, ts.URL+"/api/live/stream?token="+srv.Token(), nil)
	require.NoError(t, err)
	...
	req2, err := http.NewRequest(http.MethodGet, ts.URL+"/api/live/stream?token="+srv.Token(), nil)
	require.NoError(t, err)
